...
class application {
  ...
  protected function _search_for_controller(): void {
    ...
    $possibleControllers = [
      sprintf('%s/controller/%s.php', $this->rootPath, $this->url_controller),
      sprintf('%s/controller/%s.php', $this->rootPath, $this->defaultController),    // invalid URL, so show home/index
      sprintf('%s/controller/%s.php', $this->rootPath, config::$DEFAULT_CONTROLLER),  // invalid URL, so home/index
    ];

    $controllerFile = null;
    foreach ($possibleControllers as $path) {

      if (file_exists($path)) {

        $controllerFile = $path;
        if (self::$debug) logger::debug(sprintf('<controller : %s> %s', $path, __METHOD__));
        break;
      }
    }
    ...
    if (!$controllerFile) {
      ...
      if (self::$debug) logger::debug(sprintf('<checking for system default controller> %s', __METHOD__));
    ...
    if (!file_exists($possibleControllers[0])) {

      /**
       * we are not serving from a separate controller, 
       * we are running a method from the default controller
       */
